...
export const ContentScriptApp: React.FC<ContentScriptAppProps> = ({
  ...
  const handleAnalyze = async (): Promise<void> => {
    ...
    try {
      ...
      const contentType = domain.includes("news")
        ? "news"
        : domain.includes("blog")
          ? "blog"
          : domain.includes("cafe")
            ? "social"
            : undefined;
  ...
  const handleHighlightClick = (highlight: HighlightClickData): void => {
    ...
    if (sectionId) {
      // 해당 섹션 펼치기
      const sectionHeader = sidebarContainer.querySelector(
        `[data-section="${sectionId}"] .section-header`
      ) as HTMLElement;
      if (
        sectionHeader &&
        !sectionHeader.parentElement?.classList.contains("expanded")
      ) {
        sectionHeader.click();
      }
      ...
      // 섹션으로 스크롤
      setTimeout(() => {
        const sectionElement = sidebarContainer.querySelector(
    ...
    if (highlight.explanation) {
      const alertMessage = [
        `🎯 ${
          highlight.type === "bias"
            ? "편향성"
            : highlight.type === "manipulation"
              ? "감정 조작"
              : highlight.type === "fallacy"
                ? "논리적 오류"
                : highlight.type === "advertisement"
                  ? "광고성"
  ...
  const handleSectionClick = (
    ...
    if (itemText && analysis) {
      ...
      const matchingHighlight = allHighlights.find(
        (h) => h.text.includes(itemText) || itemText.includes(h.text)
      );
  ...
  const allHighlights: HighlightedText[] = analysis
    ? collectAllHighlights(analysis)
    : [];
