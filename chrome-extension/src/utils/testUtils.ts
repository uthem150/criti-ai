...
export function createTestAnalysisData(): TrustAnalysis {
  return {
    overallScore: 72,
    analysisSummary:
      "일부 편향적 표현과 과장된 언어가 발견되지만, 전반적으로 신뢰할 만한 수준의 콘텐츠입니다.",
    ...
    sourceCredibility: {
      ...
      reputation: {
        description:
          "신뢰할 만한 언론사로 사실 확인 시스템을 운영하고 있습니다.",
        ...
        manipulativeWords: [
          {
            ...
            impact: "high",
            explanation:
              "독자의 감정을 자극하여 객관적 판단을 흐릴 수 있습니다.",
          },
          {
            word: "반드시",
            category: "urgency",
    ...
    logicalFallacies: [
      {
        ...
        affectedText:
          "몇 명의 사람들이 좋다고 했으니 모든 사람이 좋아할 것이다",
        position: { start: 0, end: 30, selector: "" },
        severity: "medium",
        explanation:
          "몇 명의 의견만으로 모든 사람의 의견을 단정할 수는 없습니다.",
        examples: [
          "우리 반에서 3명이 축구를 좋아하니까 모든 학생이 축구를 좋아한다고 말하는 것",
        ],
      },
      {
        type: "흑백논리",
        ...
        severity: "high",
        explanation:
          "성공과 실패 사이에는 다양한 중간 단계가 있을 수 있습니다.",
...
export function validateHighlightData(highlights: HighlightedText[]): {
  ...
  highlights.forEach((highlight) => {
    ...
    if (
      !highlight.type ||
      !["bias", "fallacy", "manipulation", "advertisement", "claim"].includes(
        highlight.type
      )
    ) {
...
export function testHighlightSystem() {
  ...
  const byType = allHighlights.reduce(
    (acc: Record<string, number>, h: HighlightedText) => {
...
export function browserHighlightTest() {
  ...
  const testTexts = [
    ...
    {
      text: "몇 명의 사람들이 좋다고 했으니 모든 사람이 좋아할 것이다",
      type: "fallacy",
    },
  ...
  console.log(
    "💡 이제 Criti AI를 실행하여 하이라이트가 제대로 작동하는지 확인하세요."
  );
}
