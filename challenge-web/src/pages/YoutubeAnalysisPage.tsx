        ...
        {analysis && !loading && (
          <S.ResultCard>
            <S.ResultLayout>
              {/* 왼쪽: 입력 및 비디오 정보 */}
              <S.LeftSection>
                <S.ResultTitle>입력한 링크의 분석 결과를 알려드릴게요.</S.ResultTitle>
                ...
                <S.CollapsibleSection>
                  <S.CollapsibleHeader onClick={() => setSourceOpen(!sourceOpen)}>
                    <S.CollapsibleTitle>
                  ...
                  <S.CollapsibleContent isOpen={sourceOpen}>
                    <S.SourceInfo>
                      <S.SourceLink>
                        {analysis.videoInfo?.channelName || 'www.doctorsnews.co.kr'}
                      </S.SourceLink>
                ...
                <S.ChartContainer>
                  <S.ChartBar>
                    <S.ChartLabel>
                      ...
                      <S.ChartLabelValue score={analysis.detailedScores.channelScore}>
                    ...
                    <S.ChartBarBackground>
                      <S.ChartBarFill 
                        ...
                        color={getScoreColor(analysis.detailedScores.channelScore)}
                      />
                    </S.ChartBarBackground>
                  </S.ChartBar>
                  ...
                  <S.ChartBar>
                    <S.ChartLabel>
                      ...
                      <S.ChartLabelValue score={analysis.detailedScores.objectivityScore}>
                  ...
                  <S.ChartBar>
                    <S.ChartLabel>
                      ...
                      <S.ChartLabelValue score={analysis.detailedScores.logicScore}>
                  ...
                  <S.ChartBar>
                    <S.ChartLabel>
                      ...
                      <S.ChartLabelValue score={analysis.detailedScores.advertisementScore}>
                  ...
                  <S.ChartBar>
                    <S.ChartLabel>
                      ...
                      <S.ChartLabelValue score={analysis.detailedScores.evidenceScore}>
                  ...
                  {analysis.warnings.map((warning, idx) => (
                    <S.AnalysisItem key={idx}>
                      <S.ItemHeader>
                        ...
                        <S.Badge severity={warning.severity}>{warning.severity}</S.Badge>
                      </S.ItemHeader>
                    ...
                    <S.AnalysisItem key={idx}>
                      <S.ItemHeader>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          {element.timestamp > 0 && (
                            <S.ItemTimestamp>{formatTime(element.timestamp)}</S.ItemTimestamp>
                          )}
                          <S.Badge severity={element.severity}>{element.severity}</S.Badge>
                        </div>
                      </S.ItemHeader>
                      <S.ItemDescription>{element.explanation}</S.ItemDescription>
                    </S.AnalysisItem>
                  ))}
                </S.AnalysisContent>
              </S.FullWidthSection>
            ...
            {analysis.biasAnalysis.emotionalBias.manipulativeWords.length > 0 && (
              <S.FullWidthSection>
                ...
                <S.AnalysisContent>
                  {analysis.biasAnalysis.emotionalBias.manipulativeWords.slice(0, 5).map((word, idx) => (
                    <S.AnalysisItem key={idx}>
                      <S.ItemHeader>
                        <S.ItemTitle>"{word.word}"</S.ItemTitle>
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          <S.ItemTimestamp>{formatTime(word.timestamp)}</S.ItemTimestamp>
                          <S.Badge severity={word.impact === 'high' ? 'high' : word.impact === 'medium' ? 'medium' : 'low'}>
                            {word.impact}
                          </S.Badge>
                        </div>
                      </S.ItemHeader>
                      ...
                      <S.ItemDescription>{word.explanation}</S.ItemDescription>
                    </S.AnalysisItem>
                  ))}
                  {analysis.biasAnalysis.emotionalBias.manipulativeWords.length > 5 && (
                    <S.EmptyState>
                      그 외 {analysis.biasAnalysis.emotionalBias.manipulativeWords.length - 5}개의 감정적 표현이 더 발견되었습니다.
                    </S.EmptyState>
                  )}
                </S.AnalysisContent>
              </S.FullWidthSection>
                  ...
                  {analysis.logicalFallacies.map((fallacy, idx) => (
                    <S.AnalysisItem key={idx}>
                      <S.ItemHeader>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          <S.ItemTimestamp>{formatTime(fallacy.timestamp)}</S.ItemTimestamp>
                          <S.Badge severity={fallacy.severity}>{fallacy.severity}</S.Badge>
                        </div>
                      </S.ItemHeader>
                      <S.ItemDescription>
                        <strong>해당 내용:</strong> "{fallacy.affectedText}"
                      </S.ItemDescription>
                      <S.ItemDescription>{fallacy.explanation}</S.ItemDescription>
                    </S.AnalysisItem>
                  ))}
                </S.AnalysisContent>
              </S.FullWidthSection>
                  ...
                  {analysis.advertisementAnalysis.indicators.map((indicator, idx) => (
                    <S.AnalysisItem key={idx}>
                      <S.ItemHeader>
                        ...
                        <S.ItemTimestamp>{formatTime(indicator.timestamp)}</S.ItemTimestamp>
                      </S.ItemHeader>
                      <S.ItemDescription>
                        <strong>근거:</strong> "{indicator.evidence}"
                      </S.ItemDescription>
                      <S.ItemDescription>{indicator.explanation}</S.ItemDescription>
                    </S.AnalysisItem>
                  ))}
                </S.AnalysisContent>
              </S.FullWidthSection>
                  ...
                  {analysis.keyClaims.map((claim, idx) => (
                    <S.AnalysisItem key={idx}>
                      <S.ItemHeader>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          <S.ItemTimestamp>{formatTime(claim.timestamp)}</S.ItemTimestamp>
